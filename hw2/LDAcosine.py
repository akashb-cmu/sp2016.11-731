...
features = pickle.load(open(FEATFILE,'rb'))
...
for (i,labeled_instance) in enumerate(labeled_instances):
    instance = labeled_instance[0]
    [hyp1,hyp2,ref] = instance
    h1_vec = get_lda_weights(hyp1)
    h2_vec = get_lda_weights(hyp2)
    ref_vec = get_lda_weights(ref)
    h1_sim = get_lda_cosine(h1_vec,ref_vec)
    h2_sim = get_lda_cosine(h2_vec,ref_vec)
    features[0][i] = numpy.concatenate((features[0][i],
                                        numpy.array([h1_sim,h2_sim])))
for (i,instance) in enumerate(unlabeled_instances):
    ...
    h1_sim = get_lda_cosine(h1_vec,ref_vec)
    h2_sim = get_lda_cosine(h2_vec,ref_vec)
    features[1][i] = numpy.concatenate((features[1][i],
                                        numpy.array([h1_sim,h2_sim])))
#for instance in unlabeled_instances:
#    parallel_instance = instance
#    print str(get_prediction(parallel_instance))
